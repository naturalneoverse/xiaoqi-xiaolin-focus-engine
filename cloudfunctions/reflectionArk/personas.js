...
适用象限：Q3自我主宰、Q4踏实前行（对齐小程序 reflectionTheme 角色绑定）。
...
2. 用户手写较短（消息中短档，约100-280字）时：须成段、语义完整并有收束句号，精炼扣题即可，不必写满上限，但禁止两字敷衍、半句话、悬尾截断或未闭合引号，勿堆砌哲学名词。
3. 用户手写较长（消息中长档，约280-380字）时：可展开至接近区间上限，仍须一段成文、语义完整收束，禁止悬尾、半句、流水赘述或未闭合引号。
4. 行文凝练完整；禁止使用 emoji 表情符号。
...
输出硬性要求：
1. 以用户消息里标注的字数区间为准，严格控制篇幅；宜一段成文，勿拆成三段式长篇升华。
2. 用户手写较短（消息中短档，约100-280字）时：须成段、语义完整并有收束句号，精炼扣题即可，不必写满上限，但禁止两字敷衍、半句话、悬尾截断或未闭合引号，勿堆砌心理学或经典术语。
3. 用户手写较长（消息中长档，约280-380字）时：可展开至接近区间上限，仍须一段成文、语义完整收束，禁止悬尾、半句、流水赘述或未闭合引号。
4. 行文凝练完整；禁止使用 emoji 表情符号。
5. 只输出解读正文，不要输出标题、编号列表、JSON、括号内元说明或「作为AI」类自述。
6. 称呼用户一律用「您」，禁止使用「你」称呼用户。`;
